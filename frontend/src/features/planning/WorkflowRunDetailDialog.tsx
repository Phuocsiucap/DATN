...
import { fetchWorkflowRunCandidatesApi, fetchWorkflowRunLogsApi, type WorkflowRun, type WorkflowCandidate, type PromptRun } from '@/commons/apis/planning'
...
export function WorkflowRunDetailDialog({
  ...
  useEffect(() => {
    if (job && open) {
      setLoading(true)
      Promise.all([
        fetchWorkflowRunCandidatesApi(job.id),
        fetchWorkflowRunLogsApi(job.id)
      ])
        .then(([c, l]) => {
          setCandidates(c)
          setLogs(l)
