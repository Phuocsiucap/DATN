...
import { fetchProjectRunCandidatesApi, fetchProjectRunLogsApi, type ProjectRun, type ProjectCandidate, type PromptRun } from '@/commons/apis/planning'
import { Sheet, SheetContent } from '@/commons/component/ui/sheet'

export function ProjectRunDetailDialog({
  job,
  open,
  onOpenChange,
}: {
  ...
  useEffect(() => {
    if (job && open) {
      setLoading(true)
      Promise.all([
        fetchProjectRunCandidatesApi(job.id),
        fetchProjectRunLogsApi(job.id)
      ])
        .then(([c, l]) => {
          setCandidates(c)
          setLogs(l)
          ...
          <div>
            <div className="flex items-center gap-2">
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${
                ['COMPLETED', 'SUCCEEDED', 'WAITING_REVIEW'].includes(job.status) ? 'bg-emerald-100 text-emerald-800' :
                ...
                <div key={log.id} className="rounded-lg bg-slate-900 overflow-hidden shadow-sm text-slate-300 font-mono text-xs border border-slate-700">
                  <div className="bg-slate-950 px-4 py-2 border-b border-slate-700 flex justify-between items-center text-slate-400">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {['COMPLETED', 'SUCCEEDED'].includes(log.status) ? <span className="text-emerald-400">SUCCESS</span> : log.status === 'FAILED' ? <span className="text-red-400">ERROR</span> : <span className="text-blue-400">{log.status}</span>}
                    </div>
                  </div>
