...
def upgrade() -> None:
    ...
    for col in [
        ...
        op.create_index(f"ix_media_workflows_{col}", "media_workflows", [col])
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(["workflow_id"], ["media_workflows.id"], ondelete="CASCADE"),
        sa.ForeignKeyConstraint(["story_id"], ["stories.id"], ondelete="SET NULL"),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint("workflow_id", "source_type", "source_id", name="uq_workflow_source_ref"),
    )
    for col in ["workflow_id", "source_type", "source_id", "content_id", "story_id", "episode_id", "role", "status"]:
        op.create_index(f"ix_workflow_sources_{col}", "workflow_sources", [col])
    ...
    for col in ["workflow_id", "content_id", "story_id", "episode_id"]:
        op.create_index(f"ix_workflow_candidates_{col}", "workflow_candidates", [col])
    ...
    for col in ["workflow_id", "part_number", "status"]:
        op.create_index(f"ix_workflow_parts_{col}", "workflow_parts", [col])
    ...
    for col in ["workflow_id", "run_type", "status"]:
    ...
    for col in ["workflow_id", "artifact_type", "status"]:
        op.create_index(f"ix_workflow_artifacts_{col}", "workflow_artifacts", [col])


def downgrade() -> None:
    for table, cols in [
        ("workflow_artifacts", ["status", "artifact_type", "workflow_id"]),
        ("workflow_runs", ["status", "run_type", "workflow_id"]),
        ("workflow_parts", ["status", "part_number", "workflow_id"]),
        ("workflow_candidates", ["episode_id", "story_id", "content_id", "workflow_id"]),
        ("workflow_sources", ["status", "role", "episode_id", "story_id", "content_id", "source_id", "source_type", "workflow_id"]),
    ]:
        for col in cols:
    ...
    for col in [
        ...
        op.drop_index(f"ix_media_workflows_{col}", table_name="media_workflows")
    op.drop_table("media_workflows")
