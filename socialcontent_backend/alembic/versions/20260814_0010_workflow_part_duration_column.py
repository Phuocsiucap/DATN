...
def upgrade() -> None:
    op.add_column("workflow_parts", sa.Column("target_duration_seconds", sa.Integer(), nullable=True))
    op.execute(
        """
        UPDATE workflow_parts
...
def downgrade() -> None:
    op.drop_column("workflow_parts", "target_duration_seconds")
