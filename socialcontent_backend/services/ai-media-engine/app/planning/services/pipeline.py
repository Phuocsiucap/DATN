...
from common.db.media_workflows import sync_project_from_plan
...
class PlanningPipeline:
    ...
    def handle_workflow_run_created(self, db: Session, message: dict[str, Any]) -> None:
        ...
        job = db.get(WorkflowRun, uuid.UUID(str(job_id))) if job_id else None
    ...
    def _run(self, db: Session, job: WorkflowRun) -> None:
        ...
        for candidate in candidates:
            ...
            payload, p_name, m_name, lat, confidence, in_tok, out_tok = self._build_plan_payload(
            ...
            cost = calculate_token_cost(m_name, in_tok, out_tok)
    ...
    def _process_approved_candidate(self, db, job, primary, plan_payload, summary, input_ref):
        ...
        project = self._create_content_plan(db, job, primary, plan_payload)

        self._stage(db, job, "CREATING_SERIES", 68)

        meta = project.metadata_json or {}
        part_count = max(1, meta.get("recommended_part_count") or 1)
        target_duration = meta.get("target_duration_seconds") or 60
        content_angle = meta.get("content_angle") or ""
            ...
            for idx, p_data in enumerate(parts_payload):
                part = WorkflowPart(
                    ...
                    payload={
                        ...
                        "content_plan_id": str(project.id),
                    },
                )
                db.add(part)
        ...
        if use_inline_article_script:
            ...
            parts_payload, sp_provider, sp_model, sp_latency, sp_in, sp_out = SeriesPlannerService().plan_series(
                mode=planner_mode,
                title=planner_title,
                summary=summary,
                source_excerpt=source_excerpt,
                plan_angle=content_angle,
                tone=tone,
            ...
            sp_cost = calculate_token_cost(sp_model, sp_in, sp_out)
            db.add(
                PromptRun(
                    user_id=job.user_id,
                    workflow_run_id=job.id,
        ...
        series.metadata_json = {**(series.metadata_json or {}), "content_plan_id": str(plan.id), "last_workflow_id": str(project.id)}
        ...
        publish(
            ...
            build_event(
                ...
                source=self.consumer_name,
                job_id=job.id,
                payload={"series_id": str(series.id), "workflow_id": str(project.id), "mode": mode},
            ),
        )

        self._stage(db, job, "BUILDING_CONTEXT", 84)
        existing_ctx_doc = self._load_existing_context(series) if mode == "CONTINUE" else None
        ctx_doc, cm_provider, cm_model, cm_latency, cm_in, cm_out = ContextManagerService().build_or_update_context(
            mode=mode,
        ...
        cm_cost = calculate_token_cost(cm_model, cm_in, cm_out)
        db.add(
            PromptRun(
                user_id=job.user_id,
                workflow_run_id=job.id,
                ...
                input_tokens=cm_in,
                output_tokens=cm_out,
                total_tokens=cm_in + cm_out,
                cost_usd=cm_cost,
        ...
        db.add(WorkflowArtifact(workflow_id=project.id, artifact_type="SERIES_CONTEXT", uri=str(result.inserted_id), status="READY", metadata_json={"series_id": str(series.id), "checksum": checksum}))
        db.flush()
        ...
        publish(
            ...
            build_event(
                ...
                source=self.consumer_name,
                job_id=job.id,
                payload={"series_id": str(series.id), "context_id": str(result.inserted_id), "workflow_id": str(project.id), "mode": mode},
            ),
        )
    ...
    def _get_active_series(self, db: Session, profile_id: uuid.UUID) -> list[dict[str, Any]]:
        series_items = (
            ...
            .filter(ContentSeries.profile_id == profile_id, ContentSeries.status.in_(["ACTIVE", "PAUSED"]))
    ...
    def _series_recent_articles(self, db: Session, series: ContentSeries, limit: int = 5) -> list[dict[str, Any]]:
        parts = (
            db.query(WorkflowPart)
            .filter(WorkflowPart.series_id == series.id)
            .order_by(WorkflowPart.updated_at.desc(), WorkflowPart.part_number.desc())
            .limit(limit * 4)
            .all()
        )
        ...
        fallback_plan = db.get( uuid.UUID(str(fallback_plan_id))) if fallback_plan_id else None
    ...
    def _part_context_payload(self, part: WorkflowPart) -> dict[str, Any]:
    ...
    def _part_source_identity(self, db: Session, part: WorkflowPart) -> tuple[str, uuid.UUID | None, uuid.UUID | None]:
    ...
    def _find_plan_for_source(
        ...
        query = db.query(MediaWorkflow).filter(MediaWorkflow.profile_id == profile_id)
        if content_id:
            plan = query.filter(MediaWorkflow.primary_content_id == content_id).order_by(MediaWorkflow.updated_at.desc()).first()
            if plan:
                return plan
        if story_id:
            return query.filter(MediaWorkflow.primary_story_id == story_id).order_by(MediaWorkflow.updated_at.desc()).first()
        return None
    ...
    def _find_existing_series(
        ...
        if story_id:
            ...
            for series in db.query(ContentSeries).filter(ContentSeries.profile_id == profile_id).order_by(ContentSeries.updated_at.desc()).limit(100).all():
                if (series.metadata_json or {}).get("source_story_id") == story_key:
                    return series
        return None

    def _load_existing_context(self, series: ContentSeries) -> dict[str, Any] | None:
        try:
    ...
    def _stage(self, db: Session, job: WorkflowRun, stage: str, progress: int) -> None:
        job.current_stage = stage
        job.progress_percent = progress
        self._sync_project_from_run(job)
        db.commit()

    def _sync_project_from_run(self, run: WorkflowRun) -> None:
        project = run.project
        if not project:
            return
    ...
    def _should_skip_ai_evaluation(self, job: WorkflowRun) -> bool:
    ...
    def _force_single_part_for_direct_request(self, job: WorkflowRun, payload: dict[str, Any]) -> None:
    ...
    def _ensure_candidates_exist(self, db: Session, job: WorkflowRun) -> list[WorkflowCandidate]:
        candidates = (
            db.query(WorkflowCandidate)
            .filter(
                WorkflowCandidate.workflow_id == job.workflow_id,
                WorkflowCandidate.eligible == True,  # noqa: E712
            )
            .order_by(WorkflowCandidate.rank_order.asc().nullslast(), WorkflowCandidate.created_at.asc())
            .all()
        )
        if not candidates and job.project:
            sources = (
                db.query(WorkflowSource)
                .filter(WorkflowSource.workflow_id == job.workflow_id, WorkflowSource.status == "ACTIVE")
                .all()
            )
            if not sources and (job.project.primary_content_id or job.project.primary_story_id):
                src = WorkflowSource(
                    workflow_id=job.workflow_id,
                    source_type="CONTENT" if job.project.primary_content_id else "STORY",
                    source_id=job.project.primary_content_id or job.project.primary_story_id,
                    content_id=job.project.primary_content_id,
                    story_id=job.project.primary_story_id,
                    role="PRIMARY",
                    status="ACTIVE",
                    score=100,
                )
    ...
    def _direct_candidates(self, db: Session, job: WorkflowRun) -> list[WorkflowCandidate]:
        candidates = self._ensure_candidates_exist(db, job)
    ...
    def _score_candidates(self, db: Session, job: WorkflowRun, strategy: SocialProfileStrategy) -> list[WorkflowCandidate]:
        import re as _re
        candidates = self._ensure_candidates_exist(db, job)
    ...
    def _candidate_facts(self, db: Session, candidate: WorkflowCandidate) -> tuple[str, str, float, str, int, int]:
        content = db.get(ContentItem, candidate.content_id) if candidate.content_id else None
    ...
    def _candidate_source_context(self, db: Session, candidate: WorkflowCandidate) -> dict[str, Any]:
        content = db.get(ContentItem, candidate.content_id) if candidate.content_id else None
        story = db.get(Story, candidate.story_id) if candidate.story_id else None
        episode = db.get(Episode, candidate.episode_id) if candidate.episode_id else None
        if story and not content and story.content_id:
    ...
    def _create_content_plan(self, db: Session, job: WorkflowRun, candidate: WorkflowCandidate, payload: dict[str, Any]) -> MediaWorkflow:
        project = job.project
        project.primary_content_id = candidate.content_id
        project.primary_story_id = candidate.story_id
        project.title = payload.get("plan_title") or project.title
        project.planning_mode = payload.get("planning_mode")
        project.metadata_json = {
            **(project.metadata_json or {}),
            ...
            "recommended_part_count": payload.get("recommended_part_count"),
            "confidence_score": payload.get("confidence_score"),
            "risk_level": payload.get("risk_flags", [{}])[0].get("severity") if payload.get("risk_flags") else None,
            "ai_reasoning": payload.get("reasoning"),
            "production_requirements": payload.get("production_requirements"),
        }
        db.add(project)
        db.flush()
        return project

    def _force_single_part_for_article(self, db: Session, job: WorkflowRun, candidate: WorkflowCandidate, payload: dict[str, Any]) -> None:
        content = db.get(ContentItem, candidate.content_id) if candidate.content_id else None
    ...
    def _should_use_inline_article_script(self, db: Session, job: WorkflowRun, candidate: WorkflowCandidate, part_count: int) -> bool:
    ...
    def _should_use_standalone_part(self, job: WorkflowRun, payload: dict[str, Any], part_count: int) -> bool:
        if part_count != 1:
            return False
    ...
    def _source_refs(self, db: Session, candidate: WorkflowCandidate, part_count: int) -> list[list[dict[str, Any]]]:
    ...
    def _save_planning_input(self, job: WorkflowRun, strategy: SocialProfileStrategy, candidates: list[WorkflowCandidate]) -> str:
        doc = {
            "workflow_run_id": str(job.id),
    ...
    def _save_planning_output(self, job: WorkflowRun, step: str, payload: dict[str, Any]) -> str:
        doc = {"workflow_run_id": str(job.id), "workflow_id": str(job.workflow_id), "step": step, "prompt_version": "v1", "raw_response": payload, "parsed_response": payload, "validation_errors": [], "created_at": datetime.now(timezone.utc)}
