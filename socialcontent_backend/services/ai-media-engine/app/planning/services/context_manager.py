...
class ContextManagerService:
    ...
    ) -> tuple[dict[str, Any], str, str, int, int, int]:
        """
        Creates or updates a narrative context document for Mongo series_contexts collection.
        Returns (context_doc, provider_name, model_name, latency_ms, input_tokens, output_tokens)
        """
...
"""
        ...
        if settings.openai_api_key:
            try:
                ...
                return context_doc, result.provider, result.model, result.latency_ms, result.input_tokens, result.output_tokens
