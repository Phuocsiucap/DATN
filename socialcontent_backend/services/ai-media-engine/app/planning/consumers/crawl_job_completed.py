...
from common.db.models import ContentItem, MediaWorkflow, ProcessingRun, WorkflowSource, SocialProfile
from common.db.session import SessionLocal
from common.events.kafka import consumer
from common.events.topics import CRAWL_JOB_COMPLETED
from app.planning.schemas.planning import WorkflowRunCreateRequest
from app.planning.services.planning import PlanningService
    ...
    for profile in profiles:
        ...
        try:
            ...
            if getattr(strategy, "auto_planning_enabled", False):
                workflow_run = planning_service.create_job(
                    db,
            ...
            if workflow_run:
                print(
                    f"[planning-orchestrator] Created content project {project.id} and planning run {workflow_run.id} for profile {profile.id} from crawl job {job_id}"
                )
