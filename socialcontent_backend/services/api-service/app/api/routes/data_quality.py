...
from common.db.models import ContentItem, KafkaTask, User
...
def quality_summary(_: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    failed_tasks = db.query(func.count(KafkaTask.id)).filter(KafkaTask.status == "FAILED").scalar() or 0
...
def quality_issues(_: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    failed_tasks = db.query(KafkaTask).filter(KafkaTask.status == "FAILED").order_by(KafkaTask.updated_at.desc()).limit(50).all()
